...
namespace dale {
bool
FormLiteralArrayParse(Units *units, Function *dfn, llvm::BasicBlock *block,
                      ...
                      int *size, ParseResult *pr)
{
    ...
    if (!array_list->is_list) {
        Error *e = new Error(UnexpectedElement, array_list,
    ...
    for (std::vector<Node *>::iterator b = (lst->begin() + 1),
        ...
        if (!el->type->isEqualTo(array_type->array_type)) {
            ...
            Error *e = new Error(IncorrectType, (*b),
    ...
    *size = (int) elements.size();
    ...
    llvm::Type *llvm_array_type = ctx->toLLVMType(array_type, node, false);
    if (!llvm_array_type) {
        return false;
    }
