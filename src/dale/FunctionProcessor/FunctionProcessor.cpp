...
bool FunctionProcessor::parseFuncallInternal(
    ...
    while (symlist_iter != lst->end()) {
        ...
        if ((param_iter != fn_ptr->type->points_to->parameter_types.end())
            ...
            ParseResult coerce;
            bool coerce_result = Operation::Coerce(gen->ctx, block,
                                               p.getValue(gen->ctx),
                                               p.type,
                                               (*param_iter),
                                               &coerce);
            ...
            call_args.push_back(p.getValue(gen->ctx));
        }
    ...
    for (int i = start; i < limit; i++) {
        ...
        if (pt->is_reference) {
            ...
            bool res = arg_refpr->getAddressOfValue(gen->ctx, &refpr);
            if (!res) {
                return false;
            }
            call_args_final[i] = refpr.getValue(gen->ctx);
        } else {
            ...
            if (!args_cast) {
                ...
                call_args_final[i] = arg_refpr->getValue(gen->ctx);
            }
        }
    }
        ...
        ParseResult *pr)
{
    ...
    if (!strcmp(name, "setf")) {
         ...
         * function call. */
        call_arg_types.push_back(gen->ctx->tr->type_bool);
        call_args.push_back(gen->ctx->nt->getLLVMFalse());
    }
    ...
    if (!gen->ctx->isOverloadedFunction(t->str_value.c_str())) {
        std::map<std::string, std::vector<Function *> *>::iterator
            iter;
        Function *fn = NULL;
        for (std::vector<NSNode *>::reverse_iterator
                rb = gen->ctx->used_ns_nodes.rbegin(),
                re = gen->ctx->used_ns_nodes.rend();
                rb != re;
                ++rb) {
        ...
        if (fn && fn->is_macro) {
            ...
            if ((b == fn->parameter_types.end())
                    || (*b)->type->isEqualTo(gen->ctx->tr->type_pdnode)) {
                bool use = false;
                int size = lst->size();
                if (fn->isVarArgs()) {
    ...
    ContextSavePoint *csp = new ContextSavePoint(gen->ctx);

    while (symlist_iter != lst->end()) {
        call_arg_nodes.push_back(*symlist_iter);
        int error_count =
            gen->ctx->er->getErrorTypeCount(ErrorType::Error);
        ...
        int diff = gen->ctx->er->getErrorTypeCount(ErrorType::Error)
                   - error_count;
        ...
        if (!res || diff) {
            ...
            if (diff) {
                errors.insert(errors.end(),
                              gen->ctx->er->errors.begin() + error_count,
                              gen->ctx->er->errors.end());
                gen->ctx->er->errors.erase(gen->ctx->er->errors.begin() + error_count,
                                    gen->ctx->er->errors.end());
            }

            call_args.push_back(NULL);
            call_arg_types.push_back(gen->ctx->tr->type_pdnode);
            ++symlist_iter;
            continue;
        }
    ...
    Function *fn =
        gen->ctx->getFunction(t->str_value.c_str(),
                         &call_arg_types,
                         &closest_fn,
                         0);
    ...
    if (!fn) {
        ...
        if (gen->ctx->existsExternCFunction(t->str_value.c_str())) {
            ...
            fn = gen->ctx->getFunction(t->str_value.c_str(),
                                  NULL, NULL, 0);
            ...
            while (miter != myarg_types.end()
                ...
                ParseResult mytemp;
                bool res = Operation::Cast(gen->ctx, block,
                           (*citer),
                           (*caiter),
                           (*miter)->type,
                           n,
                ...
                block = mytemp.block;
                call_args_newer.push_back(mytemp.getValue(gen->ctx));
            ...
            args_cast = true;
        } else if (gen->ctx->existsNonExternCFunction(t->str_value.c_str())) {
            ...
            if (!t->str_value.compare("destroy")) {
                pr->set(block, gen->ctx->tr->type_void, NULL);
                return true;
            }
    ...
    if (fn->isVarArgs()) {
        ...
        while (call_args_iter != call_args.end()) {
            if ((*call_arg_types_iter)->base_type == BaseType::Float) {
                ...
                (*call_arg_types_iter) =
                    gen->ctx->tr->type_double;
            } else if ((*call_arg_types_iter)->isIntegerType()) {
                int real_size =
                    gen->ctx->nt->internalSizeToRealSize(
                        (*call_arg_types_iter)->getIntegerSize()
                    );

                if (real_size < gen->ctx->nt->getNativeIntSize()) {
                        ...
                        (*call_args_iter) =
                            builder.CreateSExt((*call_args_iter),
                                               gen->ctx->toLLVMType(
                                                    gen->ctx->tr->type_int,
                                                              NULL, false));
                        (*call_arg_types_iter) = gen->ctx->tr->type_int;
                    } else {
                        /* Target integer is not signed - use zext. */
                        (*call_args_iter) =
                            builder.CreateZExt((*call_args_iter),
                                               gen->ctx->toLLVMType(
                                                    gen->ctx->tr->type_uint,
                                                              NULL, false));
                        (*call_arg_types_iter) = gen->ctx->tr->type_uint;
                    }
                }
            }
            ++call_args_iter;
                              ...
                              std::vector<llvm::Value*> *call_args)
{
    if (return_type->is_retval) {
        ...
        if (!pr->retval) {
            llvm::IRBuilder<> builder(block);
            llvm::Type *et =
                gen->ctx->toLLVMType(return_type, NULL, false,
                                false);
            if (!et) {
                return;
            }
            ...
            pr->retval = new_ptr;
            pr->retval_type = gen->ctx->tr->getPointerType(return_type);
        } else {
